...
class COCO:
    ...
    def myshowAnns(self, anns):
        ...
        if datasetType == 'instances':
            ...
            for ann in anns:
                c = (np.random.random((1, 3))*0.6+0.4).tolist()[0]
                cb = (np.random.random((1, 3)) * 0.6 + 0.4).tolist()[0]
                #print(c)
                if 'bbox' in ann:
                    if type(ann['bbox']) == list:
                        ...
                        print(box)
                        polyb = np.array(box).reshape((int(len(box)/2), 2))
                        bpolygons.append(Polygon(polyb))
                        bcolor.append(cb)
                if 'segmentation' in ann:
                    if type(ann['segmentation']) == list:
                        # polygon
            ...
            ax.add_collection(p)
            ##;lxl add
            #bp = PatchCollection(bpolygons, facecolor=bcolor, linewidths=0, alpha=0.4)
            #ax.add_collection(bp)
            bp = PatchCollection(bpolygons, facecolor='none', edgecolors=bcolor, linewidths=2)
            ax.add_collection(bp)
        elif datasetType == 'captions':
            for ann in anns:
                print(ann['caption'])
